...
// Cafeteria time utility functions
// Cafeteria hours: 8 AM (8) to 7 PM (19)
// Self-kiosk blocked 20 minutes before closing: 6:40 PM (18:40)
...
function isWithinCafeteriaHours() {
  const { hour, minute } = getCurrentHourMinute();
  const totalMinutes = hour * 60 + minute;
  const openingMinutes = 8 * 60; // 8:00 AM
  const closingMinutes = 19 * 60; // 7:00 PM
  return totalMinutes >= openingMinutes && totalMinutes < closingMinutes;
}

function isKioskOrderingAllowed() {
  const { hour, minute } = getCurrentHourMinute();
  const totalMinutes = hour * 60 + minute;
  const openingMinutes = 8 * 60; // 8:00 AM
  const kioskCloseMinutes = (18 * 60) + 40; // 6:40 PM (20 mins before closing)
  return totalMinutes >= openingMinutes && totalMinutes < kioskCloseMinutes;
}

function getFormattedCafeteriaHours() {
  return "8:00 AM - 7:00 PM";
}

function getKioskCutoffTime() {
  return "6:40 PM";
}

window.isWithinCafeteriaHours = isWithinCafeteriaHours;
window.isKioskOrderingAllowed = isKioskOrderingAllowed;
window.isAdminUser = isAdminUser;
window.getFormattedCafeteriaHours = getFormattedCafeteriaHours;
window.getKioskCutoffTime = getKioskCutoffTime;
