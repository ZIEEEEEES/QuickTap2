...
let adminPeriodicRefreshTimer = null
const ADMIN_AUTO_REFRESH_MS = 2000
const ADMIN_PERIODIC_REFRESH_MS = 30000
...
function initializeAdmin() {
  ...
  // Initialize monitor date inputs to today
  const today = new Date()
  const todayStr = today.toISOString().slice(0, 10)
  const monitorStart = document.getElementById('monitorStartDate')
  const monitorEnd = document.getElementById('monitorEndDate')
  if (monitorStart) monitorStart.value = todayStr
  if (monitorEnd) monitorEnd.value = todayStr
  ...
  if (adminPeriodicRefreshTimer) clearInterval(adminPeriodicRefreshTimer)
  adminPeriodicRefreshTimer = setInterval(() => {
    refreshAllRealTimeViews()
  }, ADMIN_PERIODIC_REFRESH_MS)
...
async function cleanupArchivedStaff() {
  try {
    ...
    console.error("Error in cleanupArchivedStaff:", e)
  ...
  try {
    ...
    if (staffPanelVisible) {
      ...
      const roleLogs = logs.filter((l) => {
        ...
        if (roleWanted === 'kitchen_staff') return role === 'kitchen_staff'
      ...
      // For cashier filter only, also include sales activity
      if (roleWanted === 'cashier') {
...
window.deleteArchivedStaff = async function(id) {
  ...
  try {
    ...
    // First delete related admin logs to avoid foreign key constraint
    await database.from("admin_logs").delete().eq("admin_id", id)
    // Now delete the staff
    const { error } = await database.from("staff").delete().eq("id", id)
    if (error) throw error
    showMessage("Archived user deleted.", "success")
    loadArchivedStaffList()
  } catch (e) {
    console.error("Error deleting archived staff:", e)
    showMessage("Failed to delete archived user: " + e.message, "error")
  }
}
...
window.registerStaffSubmit = async function() {
  ...
  const firstName = document.getElementById('regStaffFirstName')?.value?.trim()
  const lastName = document.getElementById('regStaffLastName')?.value?.trim()
  ...
  const user = firstName ? firstName.toLowerCase() : ''
  
  if (!id || !name || !pwd || !role) {
    showMessage('Please fill ID number, first name, last name, password, and select a role', 'error')
    return
  }
  if (pwd.length < 6) {
  ...
  try {
    ...
    showMessage('Staff registered successfully.', 'success')
    
    // Clear form
    document.getElementById('regStaffId').value = ''
    document.getElementById('regStaffFirstName').value = ''
    document.getElementById('regStaffLastName').value = ''
    document.getElementById('regStaffEmail').value = ''
    document.getElementById('regStaffPassword').value = ''
    document.getElementById('regStaffRole').value = ''
    
    loadStaffList()
  } catch (e) {
    showMessage(e.message || 'Registration failed', 'error')
  }
}
...
window.removeStaff = async function(id) {
  ...
  try {
    ...
    loadStaffList()
    loadArchivedStaffList()
    showMessage('Staff archived.', 'success')
  } catch (e) {
    showMessage(e.message || 'Failed to archive', 'error')
  }
}
...
window.restoreStaff = async function(id) {
  try {
    ...
    loadStaffList()
    loadArchivedStaffList()
    showMessage('Staff restored.', 'success')
  } catch (e) {
    showMessage(e.message || 'Failed to restore', 'error')
  }
}
...
window.renderCalendar = async (date = new Date()) => {
    ...
    try {
        ...
        const { data: pData } = await db.from("pending_orders")
            .select("date, status, type")
            .gte("date", startOfMonth.split('T')[0])
            .lte("date", endOfMonth.split('T')[0])
        pendingOrders = (pData || []).filter(o => o.type !== 'redemption')
    ...
    pendingOrders.forEach(o => {
        if (o.date) {
            if (!eventsByDate[o.date]) eventsByDate[o.date] = { hasBooking: false, hasTodo: false }
            if (o.status !== 'rejected' && o.status !== 'cancelled') eventsByDate[o.date].hasBooking = true
        }
    })
    todos.forEach(t => {
...
window.renderBookingsList = async () => {
  ...
  try {
    ...
    if (customerIds.length > 0) {
      const { data: fetchedCustomers } = await db
        .from("customers")
    ...
    if (customersData) {
      customersData.forEach(c => { 
        if (c.contact) customerMap[c.contact] = {
          name: c.name,
          email: c.email,
          phone: c.phone,
          contact: c.contact
        }
        if (c.email) customerMap[c.email] = {
          name: c.name,
          email: c.email,
          phone: c.phone,
          contact: c.contact
    ...
    filteredData.forEach((d) => {
      ...
      let customerName = d.customer_name || (custFromMap?.name) || "Guest"
      ...
      tr.innerHTML = `
        <td><strong>${customerName}</strong>${customerContact ? `<br><span style="font-size:0.85em; color:#666;">${customerContact}</span>` : ''}</td>
        <td>${dateStr}</td>
        <td>${timeStr}</td>
...
window.updateOrderStatus = async (orderId, newStatus) => {
  ...
  try {
    ...
    // Record sale if completed
    if (newStatus === 'completed') {
      const { data: order } = await db.from("pending_orders").select("*").eq("id", orderId).single()
      if (order) {
        await recordBookingSale({ ...order, type: order.type || 'preorder' })
      }
    }
...
window.renderPendingRequests = async (type = currentRequestType, statusFilter = currentRequestStatus) => {
  ...
  try {
    ...
    // Fetch customer info
    const customerIds = [...new Set(filtered.map(b => b.customer_id).filter(Boolean))]
    let customersData = []
    if (customerIds.length > 0) {
      const { data: fetchedCustomers } = await db
        .from("customers")
    ...
    filtered.forEach((r) => {
      ...
      const custFromMap = customerMap[r.customer_id]
      let customerName = r.customer_name || (custFromMap?.name) || "Customer"
      let customerContact = r.customer_id || custFromMap?.email || custFromMap?.phone || custFromMap?.contact || ""
      
      listEl.innerHTML += `
        <div class="request-card request-${isPreorder ? "preorder" : "booking"}">
          <div class="request-head">
            <div>
              <div class="request-name">${customerName}</div>
              ${customerContact ? `<div class="request-meta">${customerContact}</div>` : ''}
...
async function renderDaySchedule(selectedDate) {
  ...
  // Fetch customer info
  const customerIds = [...new Set(dayRows.map(b => b.customer_id).filter(Boolean))]
  let customersData = []
  if (customerIds.length > 0) {
    const { data: fetchedCustomers } = await db
      .from("customers")
  ...
  for (let h = startHour; h < endHour; h++) {
    ...
    if (entries.length === 0) {
      ...
      entries.forEach((e) => {
        ...
        const custFromMap = customerMap[e.customer_id]
        let customerName = e.customer_name || (custFromMap?.name) || "Customer"
        let customerContact = e.customer_id || custFromMap?.email || custFromMap?.phone || custFromMap?.contact || ""
        
        html += `
          <div class="schedule-entry ${t} ${hasBooking && hasPreorder ? "has-both" : ""}">
            <div class="entry-title">${customerName}</div>
            ${customerContact ? `<div class="entry-items" style="font-size:0.85em; color:#666;">${customerContact}</div>` : ''}
            ${t === "preorder"
